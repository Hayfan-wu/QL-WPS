...
# -*- coding: utf-8 -*-
"""
WPS 信息查询模块（只读，不执行任何操作）
==============================
供机器人 /wps_query 命令调用，返回账号状态、签到情况、任务列表、抽奖次数等。
"""
...
# 与 wps_auto.py 保持一致
ACTIVITY_NUMBER = os.getenv("WPS_ACTIVITY_NUMBER", "HD2025031821201822")
PAGE_NUMBER = os.getenv("WPS_PAGE_NUMBER", "YM2025040908558269")
...
if __name__ == "__main__":
    import sys
    ck = os.getenv("WPS_COOKIE", "")
    if not ck and len(sys.argv) > 1:
        ck = sys.argv[1]
    if not ck:
        print("请设置环境变量 WPS_COOKIE")
        sys.exit(1)
    print(WpsQuery(ck).format_report())
